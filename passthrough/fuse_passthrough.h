...
struct fuse_passthrough_operations {
	...
	/*
	 * forget() operation can be implemented by modules to release
	 * per-inode resources. It is called before freeing inode with the
	 * inode mutex held.
	 */
	...
	int (*fallocate) (const fuse_path_at &, int, off_t, off_t,
			  struct fuse_file_info *);
	/*
	 * The flag fi->passthrough_read can be set to indicate that
	 ...
	 * fi->passthrough_read flag on opendir().
	 */
...
struct fuse_passthrough_module {
	...
	virtual bool debug() { return fuse_passthrough_get_opts().debug; }
