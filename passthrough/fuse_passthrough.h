...
typedef std::shared_ptr<void> fuse_inode_state_t;
...
struct fuse_inode {
	...
	virtual bool get_module_state(
			const fuse_passthrough_module &module,
			fuse_inode_state_t &state,
			fuse_fill_state_t filler = NULL,
			void *data = NULL) const = 0;
	virtual bool set_module_state(
			const fuse_passthrough_module &module,
			fuse_inode_state_t state,
			bool excl = false) = 0;
	virtual bool clear_module_state(
			const fuse_passthrough_module &module) = 0;
...
void __print_fd_path_at(const fuse_path_at &in, const char *caller);
...
struct fuse_passthrough_operations {
	...
	int (*fallocate) (const fuse_path_at &, int, off_t, off_t,
			  struct fuse_file_info *);
	/*
	 * The flag fi->passthrough_readdir can be set to indicate that
	 ...
	 * library (or the kernel).  To intercept readdir, the module needs to
	 * implement the readdir operation and clear the fi->passthrough_readdir
	 * flag on opendir().
	 */
