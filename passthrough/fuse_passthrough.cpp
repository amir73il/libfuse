...
struct Fs : public fuse_passthrough_module {
	...
	int open_by_ino(InodePtr inode);
...
int Fs::open_by_ino(InodePtr inode)
{
	if (!fhandles)
		return 0;
	...
	int fd = open_by_handle_at(root->_fd, &fake_fh.fh, O_PATH);
	if (fd < 0)
		return fd;
...
{
	auto fd = openat(at.dirfd(), at.path(), fi->flags);
	if (fd == -1)
		return -1;
			  ...
			  size_t oper_size)
{
	...
	if (!xfs_fh.decode()) {
		...
		fs.bulkstat = fs.xfs_bulkstat_gen(fs.root->src_ino);
