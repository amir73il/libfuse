...
void Fs::get_root_fh(ino_t src_ino)
{
	...
	root->_fd = open(opts.source, O_DIRECTORY | O_RDONLY);
	if (root->_fd == -1)
		err(1, "ERROR: open(\"%s\")", opts.source);

	int mount_id;
	struct xfs_fh xfs_fh{};
	at_connectable = AT_HANDLE_CONNECTABLE;
retry:
	...
	cout << "connectable file handles "
		<< (at_connectable ? "" : "not " ) << "supported" << endl;
...
static int __do_lookup(const fuse_path_at &at, const char *name, fuse_entry_param *e)
{
	...
	if (found) { // found existing inode
		...
		if (is_connectable) {
			if (fs.debug() &&
			    parent_ino != fs.get_encoder()->parent_ino(xfs_fh.fh))
                                cerr << "DEBUG: lookup(): inode " << src_ino
                                        << " parent " << parent_ino
                                        << " updated." << endl;
			inode.src_fh = xfs_fh;
