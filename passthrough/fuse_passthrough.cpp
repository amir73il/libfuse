...
void __print_fd_path_at(const fuse_path_at &at, const char *caller)
{
	char linkname[PATH_MAX];
	int n = 0;

	if (at.dirfd() != AT_FDCWD) {
		n = readlink(at.proc_path(), linkname, PATH_MAX);
	}
...
static int open_by_fh(xfs_fh &fh)
{
	if (!fs.fhandles)
		return 0;

	int fd = open_by_handle_at(fs.root->_fd, &fh.fh, O_PATH);
	if (fd < 0)
		return fd;
...
static void pfs_init(void *userdata, fuse_conn_info *conn)
{
	...
	if (conn->capable & FUSE_CAP_SPLICE_WRITE && !fs.opts.nosplice)
...
static void pfs_getattr(fuse_req_t req, fuse_ino_t ino, fuse_file_info *fi)
{
	...
	fuse_reply_attr(req, &attr, fs.opts.timeout);
}
...
static int __do_lookup(int dirfd, const char *name, fuse_entry_param *e)
{
	memset(e, 0, sizeof(*e));
	e->attr_timeout = fs.opts.timeout;
	e->entry_timeout = fs.opts.timeout;
	...
	// 1st level subdirs keep an open fd, so don't need open_by_fh)
	// on next lookup.
	src_ino = FUSE_ROOT_ID;
#endif

	// Hold long lived fd in inode if open_by_handle() is not supported
	// but always keep long lived fd for subdirs of root
	auto keep_fd = !fs.fhandles ||
		(dirfd == fs.root->_fd && S_ISDIR(e->attr.st_mode));
...
{
	auto err = __do_lookup(at.dirfd(), at.path(), e);
	if (err) {
		errno = err;
		return -1;
	}
...
{
	auto fd = openat(at.dirfd(), at.path(), O_RDONLY);
	if (fd == -1)
		return -1;
		    ...
		    off_t off, enum fuse_fill_dir_flags flags)
{
	...
	if (plus || fs.ino32) {
		auto err = __do_lookup(buf->fd, name, &e);
		if (err) {
			errno = err;
			return 0;
		}
	...
	if (entsize > rem) {
		...
		// stash ino to forget in buf->pos
		if (e.ino)
			buf->pos = e.ino;
		return 0;
	}
...
{
	auto d = get_dir(fi);
	struct fuse_buf *buf = (fuse_buf *)_buf;
	auto p = (char *)buf->mem;
	auto size = buf->size;
	int count = 0;
			       ...
			       enum fuse_readdir_flags flags)
{
	...
	// Re-purpose I/O buffer as readdir buffer
	// fd is used for lookup of readdirplus not for output so
	// FUSE_BUF_IS_FD is not set
	buf.buf[0].mem = p;
	...
	// In either way, forget ino of entry if it did not fit into buffer.
	if (buf.buf[0].pos) {
		forget_one(req, (fuse_ino_t)buf.buf[0].pos, 1);
	}
	if (errno && res == 0) {
		fuse_reply_fd_err(req, errno);
	} else {
		       ...
		       mode_t mode, fuse_file_info *fi)
{
	...
	do_passthrough_open(req, e.ino, fi);
...
static void pfs_open(fuse_req_t req, fuse_ino_t ino, fuse_file_info *fi)
{
	...
	do_passthrough_open(req, ino, fi);
	fuse_reply_open(req, fi);
}
		...
		struct fuse_file_info *fi_out, size_t len, int flags)
{
	...
	fuse_path_at at_in(req, inode_in, "");
	fuse_path_at at_out(req, inode_out, "");
			  ...
			  size_t oper_size)
{
	...
	struct stat stat;
	auto ret = lstat(opts.source, &stat);
	if (ret == -1)
		err(1, "ERROR: failed to stat source (\"%s\")", opts.source);
	...
	fs.root->_fd = open(opts.source, O_DIRECTORY | O_RDONLY);
	if (fs.root->_fd == -1)
