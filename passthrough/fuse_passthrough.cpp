/*
  ...
  Copyright (C) 2021-2026  CTERA Networks
...
retry:
	...
	if (ret < 0 || !encoder || !decode(xfs_fh) || src_ino != xfs_fh.ino) {
		...
		// If user requested to not keep open fds, fail if file handles are not supported
		if (fs.opts.keep_fd == 0)
			errx(1, "ERROR: file handles required by user");
...
void Fs::init_root()
{
	...
	// If user requested to keep open fds, do not open by file handles
	if (fs.opts.keep_fd > 0)
		fhandles = false;
	else
				  ...
				  bool is_dir = false)
{
	...
	if (is_dir) {
		if (!fs.opts.readdir_passthrough || !fi->passthrough_read)
			return false;
	} else if (!fi->passthrough_read || !fi->passthrough_write) {
		// If kernel passthrough is enabled, but not for a specific fd,
		// use dio, because another open fd of this inode may have put
		// the inode in passthrough io mode.
		// In that case, fuse requires an explicit mention of the
		// passthrough backing_id which this fd is opting-out from
		// along side the FOPEN_DIRECT_IO flag.
...
static int __do_lookup(const fuse_path_at &at, const char *name, fuse_entry_param *e)
{
	...
	if (found) { // found existing inode
		...
		// Increment nlookup before releasing fs.m, so that a racing
		// forget_one() cannot erase the inode from under us
		inode.nlookup++;
		fs_lock.unlock();
		lock_guard<mutex> g {inode.m};
...
static void forget_one(fuse_req_t req, fuse_ino_t ino, uint64_t n)
{
	...
	inode.nlookup -= n;
	if (!inode.nlookup) {
		InodeRef inode_ref(inode_ptr, false);
...
struct fuse_passthrough_opts &fuse_passthrough_get_opts()
{
	return fs.opts;
}
