...
static bool should_redirect_folder_id(const fuse_path_at &at)
{
	auto r = cgwfs.redirect();
	fuse_state_t state;
	if (at.inode().get_state(cgwfs, state) && state &&
	    r->test_folder_id(*get_folder_id(state)))
		return true;
...
static int get_file_redirect_fd(fuse_file_info *fi)
{
	...
	if (!get_file_state(fi, cgwfs, state) || !state)
		return -1;
...
static int cgwfs_lookup(const fuse_path_at &at, fuse_entry_param *e)
{
	...
	} else if (at.inode().get_state(cgwfs, state) && state) {
		if (cgwfs.debug())
...
static cxxopts::ParseResult parse_options(int &argc, char **argv)
{
	...
	cgwfs.opts.keep_fd = !options.count("nokeepfd");
	cgwfs.opts.connected_fd = true;
