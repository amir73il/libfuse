...
// We only ever set bits in a state after allocating a fuse_inode_state_t.
			      ...
			      fill_index_ctx *ctx)
{
	...
	if (nfyfs.index_by_src_ino && inode.nodeid() != inode.ino()) {
...
static IndexState *get_index_state(ino_t ino, fuse_inode_state_t &state,
			    ...
			    const char *caller)
{
	...
	if (move) {
		...
		nodeid = ino = st.st_ino;
	} else if (inode.is_root()) {
		// Treat root as indexed
		return true;
	}

	fuse_inode_state_t state;
	auto idx = get_index_state(nodeid, state, op);
	if (!idx)
		return false;
...
static int nfyfs_lookup(const fuse_path_at &at, fuse_entry_param *e)
{
	...
	if (is_dot_or_dotdot(at.path()) ||
	    !parent.get_module_state(nfyfs, state)) {
...
void nfyfs_init(fuse_passthrough_opts &opts, string index_path, bool index_new)
{
	nfyfs.opts = opts;
